...
The easiest way is the built-in helper script — it generates the key **and** writes it
straight into `backend/.env` for you:

```bash
# From the backend/ directory (after activating your venv):
python scripts/generate_secret_key.py --write
```

Or, if you prefer to copy-paste the key yourself:

```bash
# Print the key so you can paste it into .env manually:
python scripts/generate_secret_key.py
```

You can also use standard system tools:
...
# Security — generated by: python scripts/generate_secret_key.py --write
